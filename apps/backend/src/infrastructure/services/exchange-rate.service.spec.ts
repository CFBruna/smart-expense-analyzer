...
describe('ExchangeRateService', () => {
    ...
    describe('getExchangeRate', () => {
        ...
        it('should fetch from API if not cached', async () => {
            ...
            const rate = await service.getExchangeRate('USD', 'BRL');
            expect(rate).toBe(5.5);
            expect(mockedAxios.get).toHaveBeenCalledWith(expect.stringContaining('https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/usd.json'));
        ...
        it('should fetch historical rate if date is provided', async () => {
            ...
            expect(mockedAxios.get).toHaveBeenCalledWith(`https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@${dateStr}/v1/currencies/usd.json`);
            expect(mockRedisService.set).toHaveBeenCalledWith(`exchange_rate:usd:brl:${dateStr}`, 4.8, 3600);
        });
